				...
				successF : function(data) {
					...
					if(data["data"]){
						$.each(data["data"], function(index,item) {
							var listData = '<li class="mui-table-view-cell towerli"><a class="mui-navigate-right"><div class = "tower_number">杆塔编码　<span>'
...
function getRecordByTwoerNumber(towerNumber)
{
	...
	myCommon.ajaxGet({
		...
		successF : function(data) {
			...
			//塔型获取
			if($("input[name=text_tower_type]").length > 0 && typeof(data["data"]["tower_type"]) == "object") {	
				var tower_type_list = [];
				$.each(data["data"]["tower_type"], function(index, item) {
					tower_type_list.push(item);
				});
				$("input[name=text_tower_type]").data("options", JSON.stringify(tower_type_list));
				if(tower_type_list.length > 0) {
					$("input[name=text_tower_type]").val(tower_type_list[0]);
					$("input[name=tower_type]").val(tower_type_list[0]);
				}
			}
...
{
	data["formId"] = formId;
	var data = {"userId":userId, "tokenId":tokenId, "data": data};
	myCommon.loading();
	myCommon.ajaxPost({
...
{
	var param = {"userId":userId, "jsonId":jsonId, "tokenId":tokenId};
	myCommon.loading();
	myCommon.ajaxGet({
