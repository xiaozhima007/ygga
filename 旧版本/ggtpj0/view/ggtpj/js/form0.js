...
$(document).ready(function() {
	...
	//施工列表获取
    if(readonly == false && $("input[name=sgjl]").length > 0)
    {
		var sgjl_list = [];
		...
		/**
		var param = {"userId":userId, "formId":formId, "tokenId":tokenId};
		myCommon.loading();
        myCommon.ajaxGet({
            ...
            successF : function(data) {
				myCommon.closeLoading();
                if(data["code"] != "200") {
                    muiToast('获取“施工列表”信息失败');
                    return ;
                }
                console.log("data: " + JSON.stringify(data));
                var sgjl_list = [];
                $.each(data["data"], function(index, item) {
                    //sgjl_list.push([item["jsonId"], item["sgbz"]]);
                    sgjl_list.push(item["sgbz"]);
                });
                $("input[name=text_sgjl]").data("options", JSON.stringify(sgjl_list));
            },
            errorF : function() {
...
{
	var param = {"userId":userId, "formId":formId, "towerNumber": towerNumber, "tokenId":tokenId};
	myCommon.loading();
	myCommon.ajaxGet({
